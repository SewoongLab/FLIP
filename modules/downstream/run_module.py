...
def run(experiment_name, module_name, **kwargs):
    ...
    distill_labels = args.get("distill_labels", False)
    ...
    labels_syn = torch.tensor(np.load(input_path + "labels.npy"))
    if distill_labels:
        y_true = torch.tensor(np.load(input_path + "distill_labels.npy"))
    elif alpha > 0:
        y_true = torch.tensor(np.load(input_path + "true.npy"))
